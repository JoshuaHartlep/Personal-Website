...
![Manager dashboard showing KPIs and live service feed](/images/Sushi%20POS%20Demo/manager%20interface/managerinterface%20dashboard.png)
...
![Menu management page with item grid and category filters](/images/Sushi%20POS%20Demo/manager%20interface/managerinterface%20menu%20items.png)

Adding a new item is a single form — name, description, price, category, meal period, and an image upload. The item shows up immediately in both the manager menu and the customer-facing ordering UI.

![Add new menu item form](/images/Sushi%20POS%20Demo/manager%20interface/managerinterface%20add%20new%20menu%20item.png)
...
![Dining hall table status board](/images/Sushi%20POS%20Demo/manager%20interface/managerinterface%20tables.png)

![Add new table form](/images/Sushi%20POS%20Demo/manager%20interface/managerinterface%20add%20new%20table.png)
...
![Settings page with general and security tabs](/images/Sushi%20POS%20Demo/manager%20interface/managerinterface%20settings%20and%20security.png)
...
![Customer onboarding screen showing table, party size, and dining style selection](/images/Sushi%20POS%20Demo/customer%20interface/customerinterface%20logon%20menu.png)

From there it's a two-tab layout: the menu and the current order (My Order). Customers can browse items by category, open a detail modal to read the description, and customize with modifiers before adding to cart.

![Item detail modal with modifier options](/images/Sushi%20POS%20Demo/customer%20interface/customerinterface%20adding%20item.png)

Item availability is meal-period-aware — dinner-only items don't show up during lunch service, which is controlled from the manager side in real time. AYCE pricing flows through to every item in the cart: individual items show as $0.00, with the flat-rate total displayed at the bottom.

![My Order tab showing AYCE cart with flat-rate total](/images/Sushi%20POS%20Demo/customer%20interface/customerinterface%20order%20summary.png)
...
![Order confirmation screen](/images/Sushi%20POS%20Demo/customer%20interface/customerinterface%20order%20confirmation.png)
...
Want to see it yourself? Here's a live customer view: [Customer Ordering UI (Table 5)](https://sushi-point-of-sale-interface.vercel.app/customer?table=5)
...
Clicking a row drills deeper — category → items within that category → day-of-week for those items — with a breadcrumb trail so you can always backtrack. The backend drives this through a shared filter layer that compiles UI state into SQL WHERE clauses, used by every analytics endpoint.

![The Lens overview with SVG bar chart and category drill table](/images/Sushi%20POS%20Demo/manager%20interface/managerinterface%20the%20lens.png)
...
![Compare mode showing two time periods side by side with delta and % change](/images/Sushi%20POS%20Demo/manager%20interface/managerinterface%20the%20lens%20compare%20tool.png)
...
![Drilled-down view into Sushi Rolls items with revenue and order counts](/images/Sushi%20POS%20Demo/manager%20interface/managerinterface%20the%20lens%20daily%20breakdown.png)
...
Each signal card shows the metric, date, a plain-English message (e.g. "Order count was 93% above 14-day average"), the z-score, and actual vs. expected values. Clicking a card sets the date range to that specific day and drops you directly into the Overview drill view — so a signal is always one click away from exploration.

![Signals tab showing anomaly cards with z-scores and explore links](/images/Sushi%20POS%20Demo/manager%20interface/managerinterface%20the%20lens%20signals.png)
...
- **SQL as a first-class tool** — The Lens pushed me to write real analytical SQL instead of leaning on an ORM for everything. The shared filter/condition layer was one of the cleaner architectural decisions I made: one place that translates UI state into SQL WHERE clauses, used by every analytics endpoint.
